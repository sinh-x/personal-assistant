...
1. List assigned tickets: `pa ticket list --team maintenance --status todo`
...
1. **Check in-progress tickets first** — `pa ticket list --team maintenance --status doing`. Resume if found.
2. **Claim new ticket** — If nothing in-progress, run `pa ticket list --team maintenance --status todo` and claim the next item (see §Ticket Claim Protocol).
